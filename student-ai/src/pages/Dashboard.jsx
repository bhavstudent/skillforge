...
const quotes = [
  ...
  "One problem at a time. You have got this! 🎯",
];
...
function SkeletonCard() {
  return (
    <div style={{
      ...
      <div style={{ height: 16, background: "rgba(255,255,255,0.08)", borderRadius: 8, marginBottom: 12 }}></div>
      <div style={{ height: 12, background: "rgba(255,255,255,0.05)", borderRadius: 8, width: "60%" }}></div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ problemsSolved: 0, currentStreak: 0, totalXP: 0, languagesLearned: 0 });
  ...
  const langColors = {
    Python: "#3776ab", JavaScript: "#f7df1e", Java: "#f89820",
    "C++": "#00599c", SQL: "#336791", DSA: "#ff7b00"
  };
      ...
      <div className="dashboard">
        ...
        <div className="main">
          ...
          <div className="dash-hero glass-liquid">
            ...
            <div className="dash-hero-content">
              ...
              <div className="dash-hero-actions">
                <Link to="/tasks">
                  <button className="dash-btn-primary"><span>⚡</span> Start practicing</button>
                </Link>
                <Link to="/ask">
                  <button className="dash-btn-secondary"><span>🤖</span> Ask Forge AI</button>
                </Link>
              </div>
            </div>
              ...
              <div className="badge-ring">
                <svg viewBox="0 0 80 80" width="80" height="80">
                  ...
                  <circle cx="40" cy="40" r="34" fill="none" stroke="#ff7b00" strokeWidth="8"
                    strokeDasharray="213"
                    strokeDashoffset={213 - (213 * Math.min(stats.problemsSolved, 100)) / 100}
          ...
          <div className="dash-stats">
            {[
              { icon: "🔥", val: stats.currentStreak, lbl: "Day streak", color: "rgba(255,123,0,0.15)" },
              { icon: "⚡", val: stats.totalXP?.toLocaleString() || 0, lbl: "Total XP", color: "rgba(124,58,237,0.15)" },
              { icon: "💻", val: stats.languagesLearned || 0, lbl: "Languages", color: "rgba(52,211,153,0.15)" },
              { icon: "🎯", val: stats.problemsSolved, lbl: "Problems solved", color: "rgba(6,182,212,0.15)" },
            ].map((s) => (
              <div key={s.lbl} className="dash-stat-card glass" onClick={() => navigate("/progress")}>
          ...
          <div className="dash-two-col">
            <div className="glass dash-panel">
              <div className="dash-panel-title"><span>📊</span> Your skill progress</div>
              {loading ? (
                  ...
                  {topLanguages.map((lang) => (
                    <div key={lang.name} className="dash-skill-item">
                      ...
                      <div className="dash-skill-bar">
                        <div className="dash-skill-fill" style={{ width: `${lang.percentage}%`, background: langColors[lang.name] || "#ff7b00" }}></div>
                      </div>
            ...
            <div className="glass-liquid dash-panel dash-ai-card">
              ...
              <div className="dash-ai-suggestions">
                {["What should I study today?", "Explain recursion simply", "Help me with my exams"].map((s) => (
                  <button key={s} className="dash-suggestion" onClick={() => navigate("/ask")}>{s}</button>
                ))}
              </div>
            ...
            {[
              ...
              <Link to={action.to} key={action.label}>
                <div className="dash-action-card glass">
                  <div className="dash-action-icon" style={{ background: `${action.color}20`, color: action.color }}>{action.icon}</div>
